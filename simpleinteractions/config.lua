control_key = 168

anims = {
    {dic = "gestures@f@standing@casual", anim = "gesture_bring_it_on", label = "Bring It On", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_bye_hard", label = "Bye", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_come_here_hard", label = "Come Here", category = "Gesture"},
    {dic = "missheist_agency3aig_24", anim = "agent01_conversation", label = "Conversation", category = "Misc"},
    {dic = "misscarstealfinalecar_5_ig_3", anim = "crouchloop", label = "Crouch Hide", category = "Misc"},
    {dic = "gestures@f@standing@casual", anim = "gesture_damn", label = "Damn", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_displeased", label = "Displeased", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_easy_now", label = "Easy Now", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_hello", label = "Hello", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "getsure_its_mine", label = "It's Mine", category = "Gesture"},
    {dic = "missfam4leadinoutmcs2", anim = "tracy_loop", label = "Kneel On Ground Upset", category = "Misc"},
    {dic = "mp_safehouse", anim = "lap_dance_girl", label = "Lap Dance", category = "Misc"},
    {dic = "gestures@f@standing@casual", anim = "gesture_me", label = "Me", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_head_no", label = "No", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_no_way", label = "No Way", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_pleased", label = "Pleased", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_point", label = "Point", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_shrug_hard", label = "Shrug", category = "Gesture"},
    {dic = "missexile1_cargoplane", anim = "call_radio_player2", label = "Sit And Talk On Radio", category = "Misc"},
    {dic = "gestures@f@standing@casual", anim = "gesture_what_hard", label = "What", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_why", label = "Why", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_nod_yes_hard", label = "Yes", category = "Gesture"},
    {dic = "gestures@f@standing@casual", anim = "gesture_you_hard", label = "You", category = "Gesture"},
    {dic = "anim@amb@business@coc@coc_packing_hi@", anim = "full_cycle_v1_pressoperator", label = "Operate Press", category = "Misc"},
    {dic = "missfam5_blackout", anim = "pass_out", label = "Pass Out", category = "Misc"},
    {dic = "missfbi5ig_21", anim = "holdup_hallway_saw_player_scientist", label = "Scared", category = "Misc"},
    {dic = "missfbi5ig_21", anim = "hand_up_scientist", label = "Scienist Hands Up", category = "Misc"},
    {dic = "missfbi5ig_36", anim = "_reaction_to_heli", label = "See Heli", category = "Misc"},
    {dic = "missfbi3_party_c", anim = "marijuana_loop_a_male1", label = "Smoke Weed", category = "Misc"},
    {dic = "missheist_agency3astumble_getup", anim = "stumble_getup", label = "Stumble Get Up", category = "Misc"},
    {dic = "missfam5_blackout", anim = "vomit", label = "Vomit", category = "Misc"},
    {dic = "missfam5_yoga", anim = "f_yogapose_a", label = "Yoga Pose", category = "Misc"},
	--Sitzen--
	{dic = "anim@heists@fleeca_bank@ig_7_jetski_owner", anim = "owner_idle", label = "Boden Sitzen", category = "Sitzen"},
	{dic = "amb@world_human_stupor@male@idle_a", anim = "idle_c", label = "Chillig Sitzen", category = "Sitzen"},
	{dic = "random@arrests@busted", anim = "idle_c", label = "Boden kniehen Geisel", category = "Sitzen"},
	{dic = "misslester1aig_7exit", anim = "interview_exitloop_f", label = "Sitzen", category = "Sitzen"},
	--Liegen--
	{dic = "amb@world_human_bum_slumped@male@laying_on_right_side@base", anim = "base", label = "Schlafen (Rechts)", category = "Liegen"},
	{dic = "amb@world_human_bum_slumped@male@laying_on_left_side@base", anim = "base", label = "Schlafen (Links)", category = "Liegen"},
	{dic = "amb@world_human_sunbathe@male@back@base", anim = "base", label = "Liegen auf Rücken 1", category = "Liegen"},
	{dic = "mini@cpr@char_b@cpr_str", anim = "cpr_kol_idle", label = "Liegen auf Rücken 2", category = "Liegen"},
	--Tanzen--
	{dic = "a", anim = "a", label = "		Male", category = "Tanzen"},
	{dic = "missfbi3_sniping", anim = "dance_m_default", label = "Tanz 1", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@black_madonna_entourage@", anim = "hi_dance_facedj_09_v2_male^5", label = "Tanz 2", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@black_madonna_entourage@", anim = "li_dance_facedj_15_v2_male^2", label = "Tanz 3", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_li_07_v1_male^3", label = "Tanz 4", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_mi_to_li_09_v1_male^2", label = "Tanz 5", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_mi_to_li_09_v1_male^4", label = "Tanz 6", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_li_09_v1_female^1", label = "Tanz 7", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_mi_to_hi_08_v1_female^5", label = "Tanz 8", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_li_09_v1_male^1", label = "Tanz 9", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_li_09_v1_male^2", label = "Tanz 10", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_li_09_v1_male^3", label = "Tanz 11", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_li_09_v1_male^4", label = "Tanz 12", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_li_09_v1_male^5", label = "Tanz 13", category = "Tanzen"},
    {dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_li_09_v1_male^6", label = "Tanz 14", category = "Tanzen"},
    {dic = "anim@amb@nightclub@dancers@dixon_entourage@", anim = "mi_dance_facedj_15_v1_male^4", label = "Tanz 15", category = "Tanzen"},
	{dic = "a", anim = "a", label = "		Female", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_li_09_v1_female^6", label = "Tanz 1", category = "Tanzen"},
	{dic = "mini@strip_club@private_dance@part1", anim = "priv_dance_p1", label = "Tanz 2", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_mi_09_v1_female^1", label = "Tanz 3", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_mi_09_v1_female^2", label = "Tanz 4", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_mi_09_v1_female^3", label = "Tanz 5", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_mi_09_v1_female^4", label = "Tanz 6", category = "Tanzen"},
	{dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_mi_09_v1_female^5", label = "Tanz 7", category = "Tanzen"},
    {dic = "anim@amb@nightclub@dancers@crowddance_facedj_transitions@", anim = "trans_dance_facedj_hi_to_mi_09_v1_female^6", label = "Tanz 8", category = "Tanzen"},
    {dic = "a", anim = "a", label = "		Unisex", category = "Tanzen"},
    {dic = "anim@amb@nightclub@dancers@podium_dancers@", anim = "hi_dance_facedj_17_v2_male^5", label = "Lockrer Tanz", category = "Tanzen"},
    {dic = "anim@amb@nightclub@djs@dixon@", anim = "dixn_dance_a_dixon", label = "DJ 1", category = "Tanzen"},
    {dic = "anim@amb@nightclub@djs@tale_of_us@", anim = "new_tou_sync_a_cc", label = "DJ 2", category = "Tanzen"},
    {dic = "anim@mp_player_intcelebrationfemale@dj", anim = "dj", label = "DJ 3", category = "Tanzen"},
    {dic = "anim@mp_player_intcelebrationmale@dj", anim = "dj", label = "DJ 4", category = "Tanzen"},
    {dic = "mini@strip_club@idles@dj@idle_02", anim = "idle_02", label = "DJ 5", category = "Tanzen"},
    {dic = "mini@strip_club@idles@dj@idle_04", anim = "idle_04", label = "DJ 6", category = "Tanzen"},
    
	--Interaction--
	{dic = "rcmnigel1c", anim = "hailing_whistle_waive_a", label = "Rufen", category = "Interaction"},
	{dic = "amb@code_human_in_car_mp_actions@v_sign@bodhi@rps@base", anim = "idle_a", label = "Gang Gruss", category = "Interaction"},
	
}
--{dic = "", anim = "", label = "", category = "Misc"},

-- {scen = "", label = "", category = ""},
scens = {
    {scen = "WORLD_HUMAN_GUARD_STAND_ARMY", label = "Army Standing", category = "Jobs"},
    {scen = "PROP_HUMAN_ATM", label = "ATM", category = "Misc"},
    {scen = "PROP_HUMAN_BBQ", label = "BBQ", category = "Hobby"},
    {scen = "WORLD_HUMAN_BINOCULARS", label = "Binoculars", category = "Misc"},
    {scen = "WORLD_HUMAN_WINDOW_SHOP_BROWSE", label = "Browse Shop Window", category = "Misc"},
    {scen = "PROP_HUMAN_SEAT_BUS_STOP_WAIT", label = "Bus Stop", category = "Sat"},
    {scen = "WORLD_HUMAN_CAR_PARK_ATTENDANT", label = "Car Park Attendant", category = "Jobs"},
    {scen = "WORLD_HUMAN_CHEERING", label = "Cheering", category = "Misc"},
    {scen = "WORLD_HUMAN_CLIPBOARD", label = "Clipboard", category = "Jobs"},
    {scen = "WORLD_HUMAN_AA_COFFEE", label = "Coffee", category = "Drink"},
    {scen = "CODE_HUMAN_POLICE_CROWD_CONTROL", label = "Crowd Control", category = "EMS"},
    {scen = "WORLD_HUMAN_CONST_DRILL", label = "Drilling", category = "Jobs"},
    {scen = "WORLD_HUMAN_DRINKING", label = "Drinking", category = "Drink"},
    {scen = "WORLD_HUMAN_DRUG_DEALER", label = "Drug Dealer", category = "Jobs"},
    {scen = "WORLD_HUMAN_DRUG_DEALER_HARD", label = "Drug Dealer Hard", category = "Jobs"},
    {scen = "WORLD_HUMAN_MOBILE_FILM_SHOCKING", label = "Film With Phone", category = "Misc"},
    {scen = "WORLD_HUMAN_STAND_FISHING", label = "Fishing", category = "Jobs"},
    {scen = "WORLD_HUMAN_MUSCLE_FLEX", label = "Flex", category = "Hobby"},
    {scen = "WORLD_HUMAN_BUM_FREEWAY", label = "Freeway Bum", category = "Misc"},
    {scen = "WORLD_HUMAN_GOLF_PLAYER", label = "Golf Player", category = "Hobby"},
    {scen = "WORLD_HUMAN_GUARD_PATROL", label = "Guard Patrol", category = "Jobs"},
    {scen = "WORLD_HUMAN_GUARD_STAND", label = "Guard Standing", category = "Jobs"},
    {scen = "WORLD_HUMAN_HAMMERING", label = "Hammering", category = "Jobs"},
    {scen = "WORLD_HUMAN_HANG_OUT_STREET", label = "Hang Out", category = "Misc"},
    {scen = "WORLD_HUMAN_PROSTITUTE_LOW_CLASS", label = "High Class Prostitute", category = "Jobs"},
    {scen = "WORLD_HUMAN_HIKER_STANDING", label = "Hiker Standing", category = "Misc"},
    {scen = "WORLD_HUMAN_HUMAN_STATUE", label = "Human Statue", category = "Jobs"},
    {scen = "WORLD_HUMAN_COP_IDLES", label = "Idling Cop", category = "Jobs"},
    {scen = "WORLD_HUMAN_STAND_IMPATIENT", label = "Impatient", category = "Misc"},
    {scen = "WORLD_HUMAN_JANITOR", label = "Janitor", category = "Jobs"},
    {scen = "WORLD_HUMAN_JOG_STANDING", label = "Jog On The Spot", category = "Misc"},
    {scen = "WORLD_HUMAN_GARDENER_LEAF_BLOWER", label = "Leaf Blower", category = "Jobs"},
    {scen = "WORLD_HUMAN_LEANING", label = "Lean", category = "Misc"},
    {scen = "WORLD_HUMAN_PROSTITUTE_HIGH_CLASS", label = "Low Class Prostitute", category = "Jobs"},
    {scen = "WORLD_HUMAN_VEHICLE_MECHANIC", label = "Mechanic", category = "Jobs"},
    {scen = "CODE_HUMAN_MEDIC_KNEEL", label = "Medic Kneel", category = "EMS"},
    {scen = "CODE_HUMAN_MEDIC_TIME_OF_DEATH", label = "Medic Time Of Death", category = "EMS"},
    {scen = "WORLD_HUMAN_TOURIST_MOBILE", label = "Mobile", category = "Misc"},
    {scen = "WORLD_HUMAN_MUSICIAN", label = "Musician", category = "Jobs"},
    {scen = "WORLD_HUMAN_PAPARAZZI", label = "Paparazzi", category = "Jobs"},
    {scen = "WORLD_HUMAN_PARTYING", label = "Partying", category = "Hobby"},
    {scen = "WORLD_HUMAN_PICNIC", label = "Picnic", category = "Hobby"},
    {scen = "WORLD_HUMAN_GARDENER_PLANT", label = "Planting", category = "Jobs"},
    {scen = "CODE_HUMAN_POLICE_INVESTIGATE", label = "Police Investigate", category = "EMS"},
    {scen = "WORLD_HUMAN_PUSH_UPS", label = "Push Ups", category = "Hobby"},
    {scen = "WORLD_HUMAN_SECURITY_SHINE_TORCH", label = "Shine Torch", category = "Jobs"},
    {scen = "WORLD_HUMAN_SIT_UPS", label = "Sit Ups", category = "Hobby"},
    {scen = "WORLD_HUMAN_BUM_SLUMPED", label = "Slumped", category = "Misc"},
    {scen = "WORLD_HUMAN_SMOKING", label = "Smoking", category = "Drink"},
    {scen = "WORLD_HUMAN_AA_SMOKE", label = "Smoking 1", category = "Drink"},
    {scen = "WORLD_HUMAN_SMOKING", label = "Smoking 2", category = "Drink"},
    {scen = "WORLD_HUMAN_SMOKING_POT", label = "Smoking Pot", category = "Drink"},
    {scen = "WORLD_HUMAN_STAND_FIRE", label = "Stand By Fire", category = "Misc"},
    {scen = "WORLD_HUMAN_STAND_MOBILE", label = "Stand Using Mobile", category = "Misc"},
    {scen = "WORLD_HUMAN_BUM_STANDING", label = "Standing Bum", category = "Misc"},
    {scen = "WORLD_HUMAN_STUPOR", label = "Stupor", category = "Drink"},
    {scen = "WORLD_HUMAN_SUNBATHE", label = "Sunbathe", category = "Hobby"},
    {scen = "WORLD_HUMAN_SUNBATHE_BACK", label = "Sunbathe On Back", category = "Hobby"},
    {scen = "WORLD_HUMAN_SUPERHERO", label = "Superhero", category = "Hobby"},
    {scen = "WORLD_HUMAN_SWIMMING", label = "Swimming", category = "Hobby"},
    {scen = "CODE_HUMAN_MEDIC_TEND_TO_DEAD", label = "Tend To Dead", category = "EMS"},
    {scen = "WORLD_HUMAN_TENNIS_PLAYER", label = "Tennis Player", category = "Hobby"},
    {scen = "WORLD_HUMAN_TOURIST_MAP", label = "Tourist With Map", category = "Misc"},
    {scen = "WORLD_HUMAN_BUM_WASH", label = "Washing Bum", category = "Misc"},
    {scen = "WORLD_HUMAN_STRIP_WATCH_STAND", label = "Watch Stripper", category = "Hobby"},
    {scen = "WORLD_HUMAN_MUSCLE_FREE_WEIGHTS", label = "Weights", category = "Hobby"},
    {scen = "WORLD_HUMAN_WELDING", label = "Welding", category = "Jobs"},
    {scen = "WORLD_HUMAN_YOGA", label = "Yoga", category = "Hobby"},
}
--{scen = "", label = "", category = "EMS"},
